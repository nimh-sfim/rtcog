...
class Experiment:
    def __init__(self, options, mp_evt_hit, mp_evt_end, mp_evt_qa_end):
        ...
        self.pipe = Pipeline(options, self.Nv, self.Nt, self.mask_Nv, self.mask_img, self.exp_type)        
    ...
    def compute_TR_data(self, motion, extra):
        ...
        self.exp.motion_estimates.append(motion)
        ...
        this_t_data = np.array([e[self.t] for e in self.exp.extra])
        
        self.Nv = len(this_t_data)

        if self.t > 0:
            if len(self.this_t_data) != self.Nv:
