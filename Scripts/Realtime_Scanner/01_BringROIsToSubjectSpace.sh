set -e
if [ $# -lt 3 ]; then
 echo -e "\x1B[0;34mUsage: $basename $0 EPIREFFILE ANATREF CAPS_FILE [MNITEMPLATE]\x1B[0m"
...
ANATREF=$2
CAPS_FILE=$3
GM_TYPE=Segsy_NoCer # Segsy, Segsy_NoCer, MNI
if [ $# -eq 4 ]; then
	MNITEMPLATE=$4
else
	...
	MNITEMPLATE=`echo ${AFNIHOME}MNI152_2009_template.nii.gz`
        ...
        if [[ "${GM_TYPE}" == "MNI" ]]; then        
           echo "++ INFO: MNI GM Mode selected"
...
3dcalc -overwrite -b ANAT.bc+orig -m ANAT.mask+orig -expr 'm*(b*step(b))' -prefix ANAT.bc.ns
...
echo "============================"
CAPS_FILEPREFIX=`echo ${CAPS_FILE} | awk -F'+' '{print $1}' | awk -F'.' '{print $1}'` 
3dAllineate -final linear -overwrite \
            -input          ${CAPS_FILE}         \
            -1Dmatrix_apply MNI2REF.Xaff12.1D  \
            -master         EPIREF+orig        \
            -prefix         ${CAPS_FILEPREFIX}_R4Feed.nii
...
if [[ "${GM_TYPE}" == "Segsy" || "${GM_TYPE}" == "Segsy_NoCer" ]]; then
   3dcalc -a ./Segsy/Classes+orig. -expr 'equals(a,2)' -prefix rm.GMribbon_R4Feed.nii
   3dmask_tool -input rm.GMribbon_R4Feed.nii -prefix rm.GMribbon_R4Feed.nii -overwrite -dilate_inputs 2 -2
   3dAllineate -final NN -input rm.GMribbon_R4Feed.nii -1Dmatrix_apply Anat2REF.Xaff12.1D -master EPIREF+orig. -prefix GMribbon_R4Feed.nii
   3dcalc -a GMribbon_R4Feed.nii -b EPIREF.mask_R4Feed.nii -c 'Frontier2013_CAPs_R4Feed.nii[0]' -expr 'a*b*step(abs(c))' -prefix GMribbon_R4Feed.nii -overwrite
   rm rm.GMribbon_R4Feed.nii
fi

# (10) Createing Final GM Ribbon Mask
# ===================================
if [[ "${GM_TYPE}" == "Segsy_NoCer" ]]; then
 ...
 3dAllineate -final NN -overwrite \
             ...
             -1Dmatrix_apply MNI2REF.Xaff12.1D \
             -master EPIREF.mask.nii \
             -prefix MNI_GMwithoutCerebellum_alREF.nii
  3dmask_tool -overwrite -inputs MNI_GMwithoutCerebellum_alREF.nii -dilate_inputs 6 -6 -prefix MNI_GMwithoutCerebellum_alREF.nii
  3dmask_tool -overwrite -inputs MNI_GMwithoutCerebellum_alREF.nii -dilate_inputs 1 -prefix MNI_GMwithoutCerebellum_alREF.nii
  3dcalc      -overwrite -a MNI_GMwithoutCerebellum_alREF.nii -b GMribbon_R4Feed.nii -expr 'a*b' -prefix GMribbon_R4Feed.nii
  rm MNI_GMwithoutCerebellum.nii
  rm MNI_GMwithoutCerebellum_alREF.nii
else
 3dcalc -overwrite \
       -a GMribbon_R4Feed.nii \
       -b EPIREF.mask_R4Feed.nii \
       -c ${CAPS_FILEPREFIX}_R4Feed.nii'[0]' \
       -expr 'a*b*step(abs(c))' \
       -prefix GMribbon_R4Feed.nii
fi
