...
#    """
#    [m_x,m_y,m_z] = mask_img.shape
#    d_dims = len(data_img.shape)
#    mask_v = np.reshape(mask_img.get_data(),np.prod(mask_img.header.get_data_shape()), order='F')
#    if d_dims == 3:
#        data_v = np.reshape(data_img.get_data(),np.prod(data_img.header.get_data_shape()), order='F')
#        data_v = data_v.astype('float64')
#        data_v = data_v[mask_v==1]
#    if d_dims == 4:
#        [d_x,d_y,d_z,d_t] = data_img.header.get_data_shape()
#        data_v = np.reshape(data_img.get_data(),(d_x*d_y*d_z,d_t), order='F')
#        data_v = data_v.astype('float64')
#        data_v = data_v[mask_v==1,:]
#    return data_v
...
def kalman_filter(kalmTh, kalmIn, S, fPositDerivSpike, fNegatDerivSpike):
