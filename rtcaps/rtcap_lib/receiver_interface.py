...
# Based on https://github.com/afni/afni/blob/f7a41595798762eeb11ea6939df7e3f4d7665f01/src/python_scripts/scripts/realtime_receiver.py
...
class ReceiverInterface:
    ...
    def set_signal_handlers(self):
        ...
        if self.verb > 1: log.info('++ setting signals')

        slist = [ signal.SIGHUP, signal.SIGINT, signal.SIGQUIT, signal.SIGTERM ]
        if self.verb > 2: log.info('   signals are %s' % slist)
    ...
    def clean_n_exit(self, signum):

      if self.verb > 1: log.info('++ signal handler called with signal', signum)
      ...
      if self.RTI.server_sock:
         if self.verb > 1: log.info('closing server port...')
         try: self.RTI.server_sock.close()
         except (RT.socket.error, RT.socket.timeout): pass

      if self.verb > 0: log.info('-- exiting on signal %d...' % signum)
      sys.exit(signum)