...
def process_command_line():
    ...
    parser_gen.add_argument("-b","--fwhm",    action="store", type=float, dest="FWHM",    help="FWHM for Spatial Smoothing in [mm]  [default: %(default)s]", default=4.0)
...
def main():
    ...
    [Nv,Nt] = Data.shape
    print('++ INFO: Input Masked Data Dimensions [Nv=%d,Nt=%d]' % (Nv,Nt))

    # Perform Processing Step
    outData = np.zeros((Nv,Nt))
    for t in np.arange(Nt):
        aux          = Data[:,t]
        zaux         = rt_smooth_vol(aux,MaskImg,opts.FWHM)
