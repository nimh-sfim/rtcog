...
``rt-cog`` is an open-source Python package for real-time fMRI (rtfMRI) experiments.
...
To satisfy real-time constraints, ``rt-cog`` employs a multiprocessing architecture
...
On an M4 Macbook Pro, ``rt-cog`` achieves preprocessing latencies of ~0.15 seconds
...
.. figure:: /_static/images/software_architecture.png
   :alt: rt-cog software architecture
   :align: center
   :width: 90%

   Overview of the rt-cog multiprocessing architecture.

The preprocessing engine
------------------------

``rt-cog`` builds on AFNI’s real-time fMRI infrastructure, which streams image data
...
is then transmitted to ``rt-cog`` via a socket connection for further processing.

The Pipeline
^^^^^^^^^^^^

Within ``rt-cog``, incoming volumes pass through a configurable preprocessing
...
templates to detect neural patterns of interest. ``rt-cog`` currently provides two
pattern-matching approaches:

- **Region-of-interest–based matching**, which evaluates activation *levels*
  within specified regions.
- **SVR-based matching**, which uses a support vector machine based on
...
When a hit is detected (i.e., when a TR matches a target template), ``rt-cog`` can
...
For real-time monitoring and quality assurance, ``rt-cog`` integrates with the
...
``rt-cog`` is designed to be extensible at multiple levels, including preprocessing,
...
- Adaptive and closed-loop cognitive experiments
- Real-time neurofeedback
- Online neuromodulation and clinical research applications

For more information on how to extend ``rt-cog``, refer to :doc:`customize`.
